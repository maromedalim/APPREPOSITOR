...
function trial($mac) {
    ...
    if ($count == 0) {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            $db2->exec("INSERT INTO ibo (mac_address, title, type) VALUES ('$mac', 'NOVO*** (SEM DNS, USUARIO E SENHA)', 1)");
            sleep(30); // Adicionando a pausa de 30 segundos aqui
        }
...
function defaultHandle($mac) {
    ...
    if ($count == 0) {
        $expire_date = date('Y-m-d', strtotime("+07 DAYS")); // Calcula a data atual mais 7 dias
        $db2->exec("INSERT INTO ibo (mac_address, title, expire_date, type) VALUES ('$mac', '', '$expire_date', 1)");
    }
}
